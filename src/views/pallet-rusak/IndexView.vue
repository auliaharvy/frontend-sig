  ...
  <v-container>
    <v-col md-12>
      <v-card>
        ...
        <v-card>
          <v-card-title>
            <v-btn router :to="adds.route">{{ $t("damagedPallet.add") }}</v-btn>
          ...
          >
            <!-- <template v-slot:item.actions="{ item }">
              <v-menu v-if="item.status === 0 || item.status === null">
...
export default {
  ...
  methods: {
    ...mapActions("damagedPallet", ["getDamagedPallets", "deleteDamagedPallet"]),
    editData(item) {

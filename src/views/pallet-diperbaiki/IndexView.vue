<template>
  <v-container>
    <v-col md-12>
      <v-card>
        <v-card-title>
          {{ $t("repairPallet.index") }}
        </v-card-title>
        <v-divider></v-divider>
        <v-card>
          <v-card-title>
            <v-btn v-if="$can('create repaired pallet')" router :to="adds.route">{{ $t("repairPallet.add") }}</v-btn>
                  ...
                  <v-tooltip bottom>
                    <template v-slot:activator="{ on: tooltip }">
                      <v-btn
                        ...
                        text
                        icon
                        v-if="item.status == 0 || item.status === null && $can('update repaired pallet')"
                        v-bind="attrs"
                        v-on="{ ...tooltip, ...menu }"
                      >
...
export default {
  ...
  created() {
    this.getRepairedPallets(); //LOAD DATA SJP KETIKA COMPONENT DI-LOAD
  },
  computed: {
